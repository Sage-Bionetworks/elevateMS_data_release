...
# Purpose: To collate Medication Survey tables in elevateMS project
...
target.tbl.name <- 'Medication Survey'
...
##############
# Download Medication Survey tables from synapse
##############

## v1 ('syn9877098')
medication.tbl.v1.syn <- synapser::synTableQuery(paste(
  'select * from', 'syn9877098'
  # , " where healthCode = 'f6bfd9d0-6558-41a7-95f8-e913d4312014'"
))
all.used.ids <- 'syn9877098'
medication.tbl.v1.new <- getTableWithNewFileHandles(medication.tbl.v1.syn,
                                              parent.id = parent.syn.id,
                                              colsNotToConsider = 'rawData') 

# Merge all the tables into a single one
medication.tbl.new <- medication.tbl.v1.new

# Filter based on START_DATE
medication.tbl.new <- medication.tbl.new %>% 
...
cols.types <- synapser::synGetColumns('syn9877098')$asList()
...
thisFileName <- 'data_curation/curate_medication.R'
...
medication.tbl.syn.new <- synapser::synBuildTable(name = target.tbl.name,
                                            parent = parent.syn.id,
                                            values = medication.tbl.new)
medication.tbl.syn.new$schema <- synapser::Schema(name = target.tbl.name,
                                            columns = cols.types, # Specify column types
                                            parent = parent.syn.id)
tbl.syn.new <- synapser::synStore(medication.tbl.syn.new)
